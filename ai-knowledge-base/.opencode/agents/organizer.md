...
AI 知识库助手的**整理 Agent**（spec §2.3）。负责将分析结果去重合并、格式化为标准知识条目 JSON、按日期分类存入 `knowledge/articles/`，是流水线的最终输出节点。
...
| 工具 | 用途 | spec 依据 |
|------|------|-----------|
| Read | 读取 `knowledge/raw/` 原始数据和分析 Agent 产出（已标注） | §2.3 读已标注 |
| Grep | 搜索已有知识条目，检测重复 id 和 URL | § 协作契约·数据去重 |
| Glob | 查找 `knowledge/articles/` 下已有文件 | — |
| Write | 将格式化后的知识条目写入 `knowledge/articles/` | §2.3 整理成 MD |
...
1. **去重检查** — 对比已有知识条目的 `id` 和 `source_url`，跳过已存在的条目并标记 `status: duplicate`（§ 协作契约·数据去重）
2. **格式化为标准 JSON**（§2.3 整理成 MD） — 按 AGENTS.md §5 定义的知识条目结构，将采集+分析数据组装为标准 JSON 对象
3. **分类存储** — 按日期分文件存入 `knowledge/articles/`，文件命名规范：`{date}-{source}-{slug}.json`（§ 协作契约·数据怎么传：文件系统）
...
- organizer 若失败，上游结果不丢失（collector/analyzer 已有产出）
- 上游 analyzer 未成功 → organizer 跳过
- 重跑策略（§ 协作契约·重跑策略）：`python main.py --stage organize --force`
- 进度追踪（§ 协作契约·进度追踪）：流水线日志记录每个文件的写入状态
...
- [ ] 原始数据(`knowledge/raw/`)未被修改（§ 协作契约·数据怎么传：文件系统，不覆写 raw）
